...
Zmiany tylko dokumentacyjne (`README.md`, `images/`, `data/`, `Makefile`, zmiany wersji w `frontend/package.json`) pomijają pipeline automatycznie, a dodanie `[skip ci]` do treści commita pomija go ręcznie. Użyj przycisku **Run workflow** w zakładce Actions, aby wdrożyć ręcznie, gdy run został pominięty.
