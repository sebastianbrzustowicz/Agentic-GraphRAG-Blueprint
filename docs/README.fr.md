...
Les changements purement documentaires (`README.md`, `images/`, `data/`, `Makefile`, changements de version dans `frontend/package.json`) ignorent automatiquement le pipeline, et ajouter `[skip ci]` au message de commit l'ignore manuellement. Utilisez le bouton **Run workflow** dans l'onglet Actions pour déployer manuellement lorsqu'une exécution a été ignorée.
